...
Email digests require the `NOTIFICATION_STORE` KV binding and a configured email provider. Production uses Brevo through `BREVO_API_KEY` and an active `BREVO_FROM_EMAIL`; Resend remains an optional fallback. The Worker verifies the Firebase ID token before storing a subscription, derives the recipient address from Firebase rather than trusting client input, and runs the daily cron at 07:00 UTC. Weekly subscriptions are sent on Mondays. `EMAIL_DAILY_SEND_LIMIT` stops delivery before the configured provider allowance is exhausted.
